...
#        print(net_connect.send_command("show arp"))
        ...
        if ccm_test.find(" Up ") != -1:
            ...
            print('#' * 40)
            pprint('Test NOT Successful, Service OAM link NOT established')
...
with open(master['mrv_device']) as f2:
    ...
    ### From the Master file read service variables.   These variables fully define the service to be configured
        ...
        mrv_file_path = "//cal.dom/dfs/shared/networkdesign/Test/" + my_vars['service_name'] + "/" + mrv_service_file
        directory = os.path.dirname(mrv_file_path)
        print (directory)
        print (mrv_file_path)
        try:
            os.stat(directory)
        ...
        with open(mrv_file_path, 'w') as f3:
        ...
        pe_file_path = "//cal.dom/dfs/shared/networkdesign/Test/" + my_vars['service_name'] + "/" + pe_service_file
        directory = os.path.dirname(mrv_file_path)
        print (directory)
        print (pe_file_path)
        try:
        ...
        while a < 6:
            ...
            if my_vars[agg] is True and my_vars[agg_device] != 'n/a':
                ...
                agg_file_path = "//cal.dom/dfs/shared/networkdesign/Test/" + my_vars['service_name'] + "/" + agg_service_file
                directory = os.path.dirname(mrv_file_path)
                print (directory)
                print (agg_file_path)
                try:
            ...
            a = a + 1
          #
        if master['deploy_service'] is True:
...
#show status of "VPLS"
             #try:
            with Device(host=my_vars['pe_device'], user='wjohnston', password=PASSWORD2, port=22) as dev:
